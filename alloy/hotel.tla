...
define
\* добавлю просто что-нибудь пока
\* проверяем первую комнату на соответствие с гостем
\* если уж заселено, то соответственная информация
check == (Guest[1] /= 0) => Occupant[Guest[1]] = 1
end define;
 
 
\* берём информацию о госте через self номер
...
\* BEGIN TRANSLATION
\* Process variable tmpRoom of process CheckinPar at line 27 col 14 changed to tmpRoom_
